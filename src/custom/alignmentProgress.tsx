...
function getThresholdSections(progressPercent: number) {
  const p = Math.max(0, Math.min(progressPercent, 100));

  const red = Math.min(p, 50);
  const orange = Math.max(0, Math.min(p - 50, 35));
  const green = Math.max(0, Math.min(p - 85, 15));
...
export default function AlignmentProgress({ score }: AlignmentProgressProps) {
  const percent = score * 100;
  const { red, orange, green } = getThresholdSections(percent);
